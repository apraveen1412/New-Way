...
export default function Message({AIres, userQuery}){
    ...
    if(sourcesIndex !== -1){ 
        ...
        const sourceText = AIres.slice(sourcesIndex + 1, sourceEnd);
            ...
            <div className="aiResponse">
                ...
                <div className = "resSources">
                    ...
                    {sources?.map((source, index)=>{
                        return <a key={index} href={source.url} className='sourceLinks'>{source.title}</a>
                    })}
                </div>
                <div className = "resFollowUps">
                    {followups?.length > 0 ? <h5>Follow ups</h5> : null}
                    {followups?.map((followup, index)=>{
                        return <button key={index} className='followUpBtns'>{followup}</button>
                    })}
                </div>
            </div>
        </div>
